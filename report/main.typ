...
StyleGAN 进一步将输入潜变量映射到中间潜空间，并通过 AdaIN 控制每层样式，同时使用 learned constant、per-layer noise 与 style mixing 提升生成质量和可控性 @karras2019stylegan。本文实现的是一个精简化的 StyleGAN-lite。CycleGAN 面向非配对图像翻译任务，核心是 cycle consistency @zhu2017cyclegan，由于时间限制以及任务目的不同，本文仅将其作为不采用方案的参考。
...
DCGAN 生成器输入 $z in RR^128$，首先经线性层投影到 $4 times 4 times 512$ 特征图，然后通过 4 个 ConvTranspose2d 上采样块生成 $64 times 64 times 3$ RGB 图像。中间层使用 BatchNorm 和 ReLU，输出层使用 Tanh。
...
StyleGAN-lite 引入 StyleGAN 的核心思想：mapping network 将 $z$ 映射为 $w$，生成器从 learned constant 开始，通过 StyledConv：Conv、Noise、LeakyReLU、AdaIN 逐步生成图像。训练时开启 style mixing，用两个 latent 在随机层切换样式。
...
#figure(
    draw-network(
        ...
        connections: (
            (from: "w", to: "s4", type: "skip", mode: "air", pos: 4, label: "style", touch-layer: true),
            (from: "w", to: "s8", type: "skip", mode: "air", pos: 4, touch-layer: true),
            (from: "w", to: "s16", type: "skip", mode: "air", pos: 4, touch-layer: true),
            (from: "w", to: "s32", type: "skip", mode: "air", pos: 4, touch-layer: true),
        ),
        palette: "cold",
        show-legend: true,
        show-relu: true,
...
StyleGAN-lite-v2 的目标是在保持现有较小规模的前提下修复初版模型暴露出的模式崩溃问题。其改动包括：
...
对于多样性，最终评估对三种模型均计算 MS-SSIM @wang2003msssim。MS-SSIM 越低表示图像之间结构越不相似，因此本文同时报告 $1 - "MS-SSIM"$ 作为 diversity score。
...
#figure(
    table(
        ...
        [DCGAN], [`best_fid.pt`], [10000], [FID, IS, MS-SSIM],
        [StyleGAN-lite], [`best_fid.pt`], [10000], [FID, IS, MS-SSIM],
...
#figure(
    table(
        ...
        [DCGAN], [10000], [24.0600], [2.4465 ± 0.0471], [0.2125], [0.7875],
        [StyleGAN-lite], [10000], [101.6410], [1.8310 ± 0.0248], [0.3176], [0.6824],
...
#figure(
    image("assets/final_diversity_bar.png", width: 100%),
    caption: [三种模型最终 diversity score 对比。Diversity score 定义为 $1 - "MS-SSIM"$，越高表示生成样本间结构相似度越低。],
)
...
old StyleGAN-lite 后期 `loss_g` 升高到约 5.22，`loss_d` 降至约 0.36，说明判别器相对生成器过强。StyleGAN-lite-v2 采用 non-saturating loss 后，loss 数值尺度与 BCE 路径不同，不能直接和 DCGAN 数值比较；更重要的是其 FID 曲线持续下降。最终 diversity score 中，old StyleGAN-lite 为 0.6824，明显低于 DCGAN 的 0.7875 和 StyleGAN-lite-v2 的 0.8010，与其采样图中重复脸现象一致。
...
StyleGAN-lite-v2 的改进针对这些现象展开。R1 regularization 限制判别器在真实图像附近的梯度，TTUR 降低判别器学习率，EMA 平滑生成器权重，minibatch stddev 帮助判别器感知批内多样性不足，双 StyledConv 则增强每个分辨率的 synthesis 表达能力。最终 FID 从 old StyleGAN-lite 的 101.64 降到 12.78，并且优于 DCGAN 的 24.06；diversity score 也从 old StyleGAN-lite 的 0.6824 提升到 0.8010。
